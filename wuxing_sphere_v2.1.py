...
- 新增色温T维度输出
- 权重对齐司辰: 年15% 月30% 日40% 时15%

理论基础:
- 球体 = 土（整颗球都是土）
- 中轴 = 南北极轴 = 土轴/明度轴（L: 0=南极黑 → 0.5=赤道灰 → 1=北极白）
- 四行在黄道面: 木≈120°, 火=0°/8°, 金≈210°/202°, 水≈270°/262°
...
class PillarColor:
    ...
    def to_hsl(self) -> HSL:
        ...
        if self.H is None:
            # 土：中轴灰，S趋近于0，由L决定明暗
            return HSL(h=0, s=0.05, l=self.L).clamp()
        return HSL(h=self.H, s=self.S, l=self.L).clamp()
...
class SichenPillarMapper:
    ...
    def _load_json(self, json_path: Optional[str] = None):
        ...
        self._pillar_weights = data.get("pillar_weights", self.PILLAR_WEIGHTS)
    ...
    def lookup_gan_zhi(self, gan: str, zhi: str) -> PillarColor:
        ...
        """
        ganzhi = gan + zhi
        return self.lookup_pillar(ganzhi)
...
class WuxingSphereV21:
    ...
    def compute_pillar_color(self, gan: str, zhi: str, pillar_type: str = "day") -> Dict:
        """计算单柱色彩（查表）"""
    ...
    def compute_lifetime_color(self, bazi_data: Dict) -> Dict:
        ...
        for pt in ["year", "month", "day", "hour"]:
            if pt in birth:
                p = birth[pt]
                pillar_colors[pt] = self.compute_pillar_color(p["gan"], p["zhi"], pt)

        # 大运色彩
        da_yun_colors = []
        for i, dy in enumerate(da_yun_list):
            intensity = 0.2 + 0.1 * min(i, 3)
            color = self.compute_da_yun_color(birth_color, dy, intensity)
            da_yun_colors.append(color)

        # 五行平衡估算（基于四柱各柱五行）
        wuxing_balance = self._estimate_wuxing_balance(pillar_colors)

        return {
            "birth_color": birth_color["hsl_hex"],
            "birth_hsl": {
...
class SelfTest:
    ...
    def run_all(self) -> bool:
        print("=" * 60)
        print("WuxingSphereV2.1 自检报告（司辰JSON精确映射版）")
        print("=" * 60)
        ...
        # 3. 单柱查表：水=蓝紫
        pc3 = self.engine.mapper.lookup_pillar("壬申")
        self._check("壬申 H=270°(蓝紫)", pc3.H == 270, f"H={pc3.H}")

        # 4. 单柱查表：土=null
        pc4 = self.engine.mapper.lookup_pillar("戊子")
        self._check("戊子 H=null(土)", pc4.H is None, f"H={pc4.H}")
        ...
        self._check("大运色彩计算", dy_color["hsl_hex"].startswith("#"))
        ...
        # 这两个的加权平均应该靠近红色区域，而不是反方向
        self._check("圆形H平均合理", mixed["H"] is not None)
        ...
        tu_birth = self.engine.compute_birth_color(tu_bazi)
        self._check("土柱可计算", tu_birth["L"] is not None)
...
if __name__ == "__main__":
    ...
    if all_pass:
        print("\n✓ 所有自检通过")
        print("\n" + "=" * 60)
        print("梓涵八字色彩报告（壬申 己酉 丁未 丙午）")
        print("=" * 60)

        engine = WuxingSphereV21()
